...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    return NextResponse.json({ leadId, intelligence: lead.intelligence ?? buildLeadIntelligence(lead) });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to generate intelligence." }, { status: 500 });
  }
}
