...
function fallbackContacts(phone?: string | null, email?: string | null): LeadContact[] {
  return [{
    id: "primary",
...
function normalizeContacts(value: unknown, phone?: string | null, email?: string | null): LeadContact[] {
  if (!Array.isArray(value)) return fallbackContacts(phone, email);

  const contacts = value
    .filter((item) => item && typeof item === "object")
    .map((item) => {
      const record = item as Partial<LeadContact>;
...
export function LeadExecutionWorkspace({ lead }: LeadExecutionWorkspaceProps) {
  ...
  const [contacts, setContacts] = useState<LeadContact[]>(fallbackContacts(lead.phone, lead.email));
  ...
  const persistContacts = async (nextContacts: LeadContact[]) => {
